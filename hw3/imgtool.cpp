...
#include "spectrum.h"
#include "parallel.h"
#include "transform.h" // TODO: check if I should be adding this
#include "cameras/lightfield.h" // TODO: check if I should be adding this
extern "C" {
...
commands: assemble, cat, convert, diff, info, makesky, lightfield
...
lightfield options:
    ...
    --camerapos <x> <y> <z>                 Position of the desired camera for
                                            ...
                                            camera plane as z = 0)
    --filterwidth <fw>                      Radius of the reconstruction filter (in
...
int lightfield(int argc, char *argv[]) {
    ...
    Float D2C = abs(cameraPos.z);
    Float C2F = abs(focalPlaneZ);
    Float D2F = D2C + C2F;

    int xDcams = camerasPerDim.x;
    int yDcams = camerasPerDim.y;

    Float gridSizeX = griddim.pMax.x - griddim.pMin.x; // in meters
    Float gridSizeY = griddim.pMax.y - griddim.pMin.y; // in meters

    Float camSizeX = gridSizeX / float(xDcams); // in meters
    Float camSizeY = gridSizeY / float(yDcams); // in meters

    Float m = -2 / (filterWidth*filterWidth);
    Float b = 2 / filterWidth;

    Float outputfovRads = outputfov * M_PI / 180.f;
    Float inputfovRads = inputfov * M_PI / 180.f;

    Float maxT = cameraPos.y + D2C * tan(outputfovRads);
    Float minT = cameraPos.y - D2C * tan(outputfovRads);
    Float maxS = cameraPos.x + D2C * tan(outputfovRads);
    Float minS = cameraPos.x - D2C * tan(outputfovRads);

    Float maxG = cameraPos.y + D2F * tan(outputfovRads);
    Float minG = cameraPos.y - D2F * tan(outputfovRads);
    Float maxF = cameraPos.x + D2F * tan(outputfovRads);
    Float minF = cameraPos.x - D2F * tan(outputfovRads);

    Point2f focalSize = Point2f((maxF - minF) / outputDim.x, (maxG - minG) / outputDim.y);

    Transform ScreenToRaster, RasterToScreen, RasterToCamera, CameraToScreen, CameraToRaster;
    CameraToScreen = Perspective(inputfov, 1e-2f, focalPlaneZ);
    // RasterToScreen = Inverse(ScreenToRaster);
    // RasterToCamera = Inverse(CameraToScreen) * RasterToScreen;
    // CameraToRaster = Inverse(RasterToCamera);

    for (int x = 0; x < outputDim.x; x++) {
        for (int y = 0; y < outputDim.y; y++) {
            // Get what current F and G we're at
            Float f = float(x) * focalSize.x + minF;
            Float g = float(y) * focalSize.y + minG;

            // Find what current S and T we're at
            Float curS = cameraPos.x - (cameraPos.x - f) * D2C / D2F;
            Float curT = cameraPos.y - (cameraPos.y - g) * D2C / D2F;

            // Find nearest cameras
            int s0 = int(curS / camSizeX);
            int s1 = (curS < float(s0) * camSizeX + camSizeX / 2.f) ? s0 - 1 : s0 + 1;
            s1 = (s1 < 0) ? s0 + 1 : (s1 >= xDcams) ? s0 - 1 : s1;
            if (s0 > s1) {
                int temp = s0;
                s0 = s1;
                s1 = temp;
            ...
            int t0 = int(curT / camSizeY);
            int t1 = (curT < float(t0) * camSizeY + camSizeY / 2.f) ? t0 - 1 : t0 + 1;
            t1 = (t1 < 0) ? t0 + 1 : (t1 >= yDcams) ? t0 - 1 : t1;
            if (t0 > t1) {
                int temp = t0;
                t0 = t1;
                t1 = temp;
            ...
            std::unique_ptr<RGBSpectrum[]> colors(new RGBSpectrum[4]);
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    ...
                    Point3f Dst = Point3f(float(s) * camSizeX + camSizeX / 2.f, float(t) * camSizeY + camSizeY / 2.f, 0);

                    Float distS = Dst.x - curS;
                    Float distT = Dst.y - curT;
                    Float dist = sqrt(distS*distS + distT*distT);
                    if (dist < filterWidth) dist = 0.0;
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 2; j++) {
                    ...
                    Point3f Dst = Point3f(float(s) * camSizeX + camSizeX / 2.f + minS, float(t) * camSizeY + camSizeY / 2.f + minT, 0);
                    Point3f fgPoint = Point3f(f, g, focalPlaneZ);

                    ScreenToRaster =
                        Translate(Vector3f(minF, minG, focalPlaneZ)) *
                        Scale(maxF - minF, maxG - minG, 1) *
                        Scale(float(camerasPerDim.x) / float(res.x), float(camerasPerDim.y) / float(res.y), 1) *
                        Translate(Vector3f(-(Dst.x - camSizeX / 2.f), -(Dst.y - camSizeY / 2.f), 0));

                    // Transform camera to raster
                    Point3f fgCam = ScreenToRaster(Dst);
                    fgCam = Point3f((fgCam.x + minF) / (maxF - minF) * res.x, (fgCam.y + minG) / (maxG - minG) * res.y, fgCam.z);
                    ...
                    Float distT = Dst.y - curT;
                    Float dist = sqrt(distS*distS + distT*distT);
                    if (dist < filterWidth) w = dist / totalWeight; //w = dist * m + b;

                    // Index to get color
                    int inputIndex = int(fgCam.x * res.x + fgCam.y);
                    if (inputIndex >= 0 && inputIndex < res.x * res.y) {
                        int colorIndex = int(i * 2 + j);
                        colors[colorIndex] = w * image[inputIndex];
                    }
                }
            }

            Float s = fmod(curS, camSizeX) / camSizeX;
            Float t = fmod(curT, camSizeY) / camSizeY;

            // Bilinear interpolation
            int outputIndex = int(x * outputDim.x + y);
            if (outputIndex > 0 && outputIndex < outputDim.x*outputDim.y) {
                outputImage[outputIndex] = outputImage[outputIndex] +
                    (1-s)*(1-t)*colors[0] + (1-s)*t*colors[1] + s*(1-t)*colors[2] + s*t*colors[3];
            }
        }
    }
